...
import org.jclouds.googleclouddns.functions.internal.ParseResourceRecordSets;
...
/**
 * Provides access to ResourceRecordSet via its REST API.
 *
 * @author Javier Kohen
...
public interface ResourceRecordSetApi {
    ...
    */
   @Named("ResourceRecordSets:list")
   @GET
   @Consumes(MediaType.APPLICATION_JSON)
   @Path("/managedZones/{managedZone}/rrsets")
   @OAuthScopes(DNS_READONLY_SCOPE)
   @ResponseParser(ParseResourceRecordSets.class)
   @Transform(ParseResourceRecordSets.ToPagedIterable.class)
   @Fallback(EmptyPagedIterableOnNotFoundOr404.class)
   ListPage<ResourceRecordSet> listAtMarkerInManagedZone(
   ...
   @Transform(ParseResourceRecordSets.ToPagedIterable.class)
   @Fallback(EmptyPagedIterableOnNotFoundOr404.class)
   PagedIterable<ResourceRecordSet> listInManagedZone(@PathParam("managedZone") String managedZone);
}
