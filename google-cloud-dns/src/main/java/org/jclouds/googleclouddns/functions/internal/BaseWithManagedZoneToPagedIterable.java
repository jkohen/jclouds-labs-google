 ...
 */
@Beta
public abstract class BaseWithManagedZoneToPagedIterable<T, I extends BaseWithManagedZoneToPagedIterable<T, I>> implements
   ...
   public PagedIterable<T> apply(ListPage<T> input) {
      ...
      Optional<Object> managedZone = tryFind(request.getInvocation().getArgs(), instanceOf(String.class));

      Optional<Object> listOptions = tryFind(request.getInvocation().getArgs(), instanceOf(ListOptions.class));

      assert project.isPresent() : String.format("programming error, method %s should have a string param for the project",
              request.getCaller().get().getInvokable());

      assert managedZone.isPresent() : String.format("programming error, method %s should have a string param for the managed zone",
              request.getCaller().get().getInvokable());

      return PagedIterables.advance(
              input, fetchNextPage(project.get().toString(), managedZone.get().toString(), (ListOptions) listOptions.orNull()));
   }

   protected abstract Function<Object, IterableWithMarker<T>> fetchNextPage(String projectName,
                                                                            String managedZoneName,
