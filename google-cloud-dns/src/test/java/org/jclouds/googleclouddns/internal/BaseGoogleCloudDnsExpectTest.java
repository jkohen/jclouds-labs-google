 ...
 * @author Adrian Cole
 */
public class BaseGoogleCloudDnsExpectTest<T> extends BaseRestApiExpectTest<T> {
   ...
   public BaseGoogleCloudDnsExpectTest() {
   ...
   protected <T> PagedIterable<T> toPagedIterable(ListPage<T> list) {
      return PagedIterables.onlyPage(list);
   }

   protected static Payload staticPayloadFromResource(String resource) {
      try {
         return payloadFromString(Strings2.toStringAndClose(BaseGoogleCloudDnsExpectTest.class.getResourceAsStream
                 (resource)));
      } catch (IOException e) {
         throw propagate(e);
      }
